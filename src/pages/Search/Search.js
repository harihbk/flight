    ...
    return (
        <>
            {rest?._flightdetail?.map((flightdetail, flightdetailindex) => (
                <>
                    <Box className='flightlist flightfrom' key={flightdetailindex} style={{ alignItems: 'center' }}>
                        <Box className='brand'>
                        ...
                        <Box className='time_place first'>
                            <Typography className='time1' style={{ fontSize: rest?.isCombo ? 12 : 16, fontWeight: '500' }}>{flightdetail?.flightdetaildt?.dt}</Typography>
                        ...
                        <Box className='time_place'>
                            <Typography className='time1' style={{ fontSize: rest?.isCombo ? 12 : 16, fontWeight: '500' }}>{flightdetail?.flightdetailat?.at}</Typography>
...
export default function Search({ isVisible }) {
    ...
    // skeleton
    const [tripSkeleton, setTripSkeleton] = React.useState(true)
    ...
    // booking Data
    const [flightBookingData, setFlightBookingData] = React.useState({
        start: {},
        return: {},
        departureAmont: '',
        returnAmount: ''
    ...
    const [singleBookingDat, setSingleBookingDat] = React.useState();
    ...
    React.useEffect(() => {
        ...
        * **/
        ...
        axios.post(`${process.env.REACT_APP_FLIGHT_URL}/fms/v1/air-search-all`, CreatesearchObject, { headers: headers }).then(res => {
            setTripSkeleton(false);
            console.log(res);
            if (!Object.entries(res?.data?.searchResult).length) {
            ...
            if (_return) {
                ...
                var modifieddata = _return.map((dd, i) => {
                    ...
                    if (dd.totalPriceList.length == 1) {
                        dd.totalPriceList[0].checked = false
                        dd.totalPriceList[0].totalamount = calculatetotalamount1(dd.totalPriceList[0], paxt)
                    } else if (dd.totalPriceList.length > 1) {
                        dd.totalPriceList[0].checked = false
                        dd.totalPriceList[0].totalamount = calculatetotalamount1(dd.totalPriceList[0], paxt)
    ...
    const radioChangeSingle = (parentindex, data, e, fdata) => {
        ...
        setSingleBookingDat(data);
        console.log(data);
    }
    const radiochangeevent = (parentindex, data, e, fdata) => {
        let target_id = e.target.value
        let unmutate = [...listflight];
        ...
        unmutate.map((list) => {
            // console.log(list);
            return list?.totalPriceList.map((priceList) => {
                priceList.checked = false
            })
        });
        ...
        unmutate[parentindex]['totalPriceList'][plindex].checked = true
        setListflightfilter(unmutate)
        let totalAmountObj = data.find(o => o.id === target_id);
        setFlightBookingData((prev) => ({
            ...prev,
            start: fdata,
            departureAmont: totalAmountObj?.totalamount
        }));
    }

    const radiochangeeventreturn = (parentindex, data, e, fdata) => {
        let target_id = e.target.value;
        ...
        setFlightBookingData((prev) => ({
            ...prev,
            return: fdata,
            returnAmount: totalAmountObj?.totalamount
        }));
    }
    ...
    const comboBooking = (bookingData) => {
        console.log(bookingData)

        let departureIndex = bookingData.start?.totalPriceList.findIndex(a => a.checked)
        let returnIndex = bookingData.return?.totalPriceList.findIndex(a => a.checked)
        let departureId = bookingData.start?.totalPriceList[departureIndex].id;
        let returnId = bookingData.return?.totalPriceList[returnIndex].id;
        console.log(departureId, returnId);
        navigate(`/booking/flight/${departureId},${returnId}`);
    }

    const totalAmount = (total_amount) => {
        return total_amount.toLocaleString(undefined, { minimumFractionDigits: 2 })
    }
                        ...
                        <Box className="contentWrapper" component={'div'}>
                            <Box component="form" className='inputWrapper'>
                                ...
                                <div className='bookingStripe_search'>
                                    ...
                                    <div className='departure booking_input' onClick={handleButtonClick} ref={buttonRef}>
                                        ...
                                        {isPopperOpen && (
                                            <Calendar calendarOpen={calendarOpen} _parentcalendarfuction={parentcalendarfuction} />
                                        )}
                                    </div>
                                ...
                                <Grid item md={9} className="booking_col">
                                    ...
                                    {displaystatus == 'oneway' && (
                                        <Box className={'chooseFlightSect '}>
                                            {/* {listflightfilter} */}
                                            {!_.isEmpty(flightBookingData.start) && !_.isEmpty(flightBookingData.return) && (
                                                <Box component={'div'} className='mainBookingrow' style={{ marginBottom: 15, position: 'sticky', top: 10, zIndex: 1 }}>
                                                    <Grid container>
                                                        <Grid item md={4}>
                                                            <Typography style={{ fontSize: 13, fontWeight: '500', opacity: 0.5 }}>Departure</Typography>
                                                            <Typography className='depart_place'> {flightBookingData.start?.arrival_obj?.city}  <span className='interTextDot'>.</span>  {flightBookingData.start?.dept_obj?.city}  </Typography>
                                                            <Box component={'div'} className="flight_timerow">
                                                                <Box className='time'>
                                                                    <div className='icons'>
                                                                        <img src={require('../../assets/icons/flighticon.png')} alt='flight' />
                                                                    </div>
                                                                    <Typography className='start_time timeText'>{flightBookingData.start?.dept_obj?.timing} </Typography>
                                                                    <ArrowRightAlt className='miniArrow' />
                                                                    <Typography className='end_time timeText'>{flightBookingData.start?.arrival_obj?.timing}</Typography>
                                                                </Box>
                                                                <Box className='price'>
                                                                    {/* ₹  {flight?.totalPriceList[0]?.totalamount}  */}
                                                                </Box>
                                                            </Box>
                                                            <Typography className='details_text' sx={{ marginTop: 1 }}> <b>{flightBookingData.start?.dept_obj?.name}</b> {flightBookingData.start?.flightdetails?.[0]?.flightcodefn} </Typography>
                                                        </Grid>
                                                        <Grid item md={4} >
                                                            <Typography style={{ fontSize: 13, fontWeight: '500', opacity: 0.5 }}>Return</Typography>
                                                            <Typography className='retun_place'> {flightBookingData.return?.arrival_obj?.city}  <span className='interTextDot'>.</span>  {flightBookingData.return?.dept_obj?.city}   </Typography>
                                                            <Box component={'div'} className="flight_timerow">
                                                                <Box className='time'>
                                                                    <div className='icons'>
                                                                        <img src={require('../../assets/icons/flighticon.png')} alt='flight' />
                                                                    </div>
                                                                    <Typography className='start_time timeText'>{flightBookingData.return?.dept_obj?.timing} </Typography>
                                                                    <ArrowRightAlt className='miniArrow' />
                                                                    <Typography className='end_time timeText'>{flightBookingData.return?.arrival_obj?.timing}</Typography>
                                                                </Box>
                                                                <Box className='price'>
                                                                    {/* ₹ {flight?.frmt[1]?.dept_obj?.name}  */}
                                                                </Box>
                                                            </Box>
                                                            <Typography className='details_text' sx={{ marginTop: 1 }}> <b>{flightBookingData.return?.dept_obj?.name}</b> {flightBookingData.return?.flightdetails?.[0]?.flightcodefn} </Typography>
                                                        </Grid>
                                                        <Grid item md={3} >
                                                            <Typography className='total_price'>
                                                                <span>₹</span>
                                                                {totalAmount(flightBookingData.departureAmont + flightBookingData.returnAmount)}
                                                                {/* {helpers.calculatetotalamount(totaldata, _paxtypeget)} */}
                                                            </Typography>
                                                            <Typography className='details_text'>Fare Details</Typography>
                                                            <Button variant='contained' className='color_primary booknow_btn 1' onClick={() => comboBooking(flightBookingData)}>Book Now</Button>
                                                        </Grid>
                                                    </Grid>
                                                </Box>
                                            )}
                                                    ...
                                                    <Grid item md={12}>
                                                        <Box className='cardBox'>
                                                            ...
                                                            {listflightfilter && listflightfilter.map((data, i) => (
                                                                <Box className='flightitem single_item'>
                                                                    ...
                                                                    <Grid spacing={2} container>
                                                                            ...
                                                                            <Box className='timeandDetails'>
                                                                                ...
                                                                                <span>
                                                                                    {data?.totalPriceList.map((totaldata, totindex) => (
                                                                                        <Box className='price comboview_price' >
                                                                                            <Typography variant="h6" sx={{ fontSize: 11, color: '#999',  textAlign :'right'  }} >{cabinClassget}</Typography>
                                                                                            <Typography className='priceText'>
                                                                                                <Box className='single_line'>
                                                                                                    <Box style={{ marginRight : 5 }}>₹ {calculatetotalamount(totaldata)} </Box>
                                                                                                    <span>
                                                                                                        <Radio
                                                                                                            checked={totaldata?.checked}
                                                                                                            id={totaldata?.id}
                                                                                                            value={totaldata?.id}
                                                                                                            onChange={(e) => radioChangeSingle(i, data?.totalPriceList, e)}
                                                                        ...
                                                                        <Grid md={2} item>
                                                                            <Box>
                                                                                ...
                                                                                <Typography sx={{ fontSize: 11, color: '#213bd4', marginTop : 1 }}>
                                            ...
                                            {splitType &&
                                                <Grid container spacing={2} className="comboview">
                                                    <Grid item md={6}>
                                                        <Box className='cardBox'>
                                                            <Box style={{ padding: 10, borderBottomWidth: 1, borderColor: '#ccc', borderBottomStyle: 'solid' }}>
                                                                <Typography className='journerydate journey_start' component={'div'}>
                                                            ...
                                                            {/* flights  one way  */}
                                                            {listflightfilter && listflightfilter.map((data, i) => (
                                                                <Box className='flightitem comboview_item'>
                                                                    <Box className='flight_brand'>
                                                                        <img src={require('../../assets/icons/flighticon.png')} alt='flight' /> {data?.dept_obj?.name ?? '-'}
                                                                    </Box>
                                                                                        ...
                                                                                        <Box className='price comboview_price' >
                                                                                            ...
                                                                                            <Typography className='priceText'>
                                                                                                <Box style={{ textAlign: 'right' }}>
                                                                                                    <span style={{ marginRight: 5 }}>₹ {calculatetotalamount(totaldata)}</span>
                                                                                                    <span>
                                                                                                        <Radio
                                                                                                            //    checked={flightBookingData.start?.id == totaldata?.id}
                                                                                                            checked={totaldata.checked}
                                                                                                            id={totaldata?.id}
                                                                                                            value={totaldata?.id}
                                                                                                            onChange={(e) => radiochangeevent(i, data?.totalPriceList, e, data)}
                                                                                                            name={"flights-" + totaldata?.id}
                                                                                                            inputProps={{ 'aria-label': 'A' }}
                                                                                                            sx={colorsRadio}
                                                                                                        />
                                                                    ...
                                                                    <Box className='seatDetails_row'>
                                                                        <Typography sx={{ fontSize: 12, color: '#f43a00' }}>Seats left {data?.totalPriceList[0]?.fd?.ADULT?.sR}</Typography>
                                                                        <Typography className={`fdetails ${tabValue}`} onClick={() => tabValue == i ? TabChange('-1') : TabChange(i)}> {'Flight Details'} <KeyboardArrowDown className='down' /></Typography>
                                                                    </Box>
                                                                    ...
                                                                    {tabValue == i && (
                                                                        <Box className='flight_detail_bot tab'>
                                                                            <TabsUnstyled defaultValue={0}>
                                                                                ...
                                                                                <TabPanelUnstyled value={0}>
                                                                                    <Flightdetails _flightdetail={data?.flightdetails} isCombo={splitType} />
                                                                                </TabPanelUnstyled>
                                                                                <TabPanelUnstyled value={1}>
                                                                                    <Faredetails value={data?.totalPriceList} _paxtypeget={paxtypeget} />
                                                                                </TabPanelUnstyled>
                                                        ...
                                                        <Box className='cardBox'>
                                                            ...
                                                            {listflightfilterreturn && listflightfilterreturn.map((data, i) => (
                                                                <Box className='flightitem comboview_item'>
                                                                                        ...
                                                                                        <Box style={{ textAlign: 'right' }}>
                                                                                            ...
                                                                                            <span>
                                                                                                <Radio
                                                                                                    ...
                                                                                                    id={totaldata?.id}
                                                                                                    value={totaldata?.id}
                                                                                                    onChange={(e) => radiochangeeventreturn(i, data?.totalPriceList, e, data)}
                                                                                                    name={"flights-" + totaldata?.id}
                                                                                                    inputProps={{ 'aria-label': 'A' }}
                                                                                                    sx={colorsRadio}
                                                                                                />
                                                                    ...
                                                                    <Box className='seatDetails_row'>
                                                                        <Typography sx={{ fontSize: 11, color: '#f43a00' }}>Seats left {data?.totalPriceList[0]?.fd?.ADULT?.sR}</Typography>
